...
public class GtkPreferredJComboBoxRendererInspection extends DevKitOnlyInspectionBase
{
	...
	private static final String[] RIGHT_RENDERER_CLASS_NAMES = {
			ListCellRendererWrapper.class.getName(),
			ColoredListCellRenderer.class.getName()
	};
	private static final String SETTER_METHOD_NAME = "setRenderer";

	private static final String MESSAGE = "Default ListCellRenderer implementations are known to cause UI artifacts under GTK+ look&feel, " +
	...
	public PsiElementVisitor buildInternalVisitor(@NotNull final ProblemsHolder holder, final boolean isOnTheFly)
	{
		return new JavaElementVisitor()
		{
			...
			public void visitMethodCallExpression(final PsiMethodCallExpression expression)
			{
				...
				if(!SETTER_METHOD_NAME.equals(method.getName()))
				{
					return;
				}
				...
				if(!InheritanceUtil.isInheritorOrSelf(aClass, comboClass, true))
				{
					return;
				}
				...
				for(String rightClassName : RIGHT_RENDERER_CLASS_NAMES)
				{
					...
					if(InheritanceUtil.isInheritorOrSelf(rendererClass, rightClass, true))
					{
						return;
					}
				}
