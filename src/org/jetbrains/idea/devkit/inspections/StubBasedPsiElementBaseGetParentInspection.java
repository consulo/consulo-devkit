...
public class StubBasedPsiElementBaseGetParentInspection extends DevKitInspectionBase
{
	...
	public ProblemDescriptor[] checkClass(@NotNull PsiClass psiClass, @NotNull InspectionManager manager, boolean isOnTheFly)
	{
		...
		final ProblemDescriptor descriptor = manager.createProblemDescriptor(ObjectUtils.assertNotNull(psiClass.getNameIdentifier()), "Default getParent() implementation is slow",
				new InsertGetParentByStubOverrideQuickFix(psiClass), ProblemHighlightType.GENERIC_ERROR_OR_WARNING, isOnTheFly);
	...
	private static class InsertGetParentByStubOverrideQuickFix extends LocalQuickFixOnPsiElement
	{
		...
		public void invoke(@NotNull Project project, @NotNull PsiFile file, @NotNull PsiElement startElement, @NotNull PsiElement endElement)
		{
			...
			final PsiClass stubBasedPsiElementClass = JavaPsiFacade.getInstance(project).findClass(StubBasedPsiElementBase.class.getName(), psiClass.getResolveScope());
			assert stubBasedPsiElementClass != null;

			final PsiMethod methodToOverride = stubBasedPsiElementClass.findMethodBySignature(createGetParentTemplateMethod(stubBasedPsiElementClass), false);
			...
			final PsiCodeBlock optimizedCodeBlock = JavaPsiFacade.getElementFactory(project).createCodeBlockFromText("{ return getParentByStub(); }", getParentMethod);
			...
			GenerateMembersUtil.insertMembersBeforeAnchor(psiClass, anchor, Collections.singletonList(new PsiGenerationInfo<PsiMethod>(getParentMethod)));
		}
	}
}