					...
					if(r instanceof PsiLiteralExpression && g instanceof PsiLiteralExpression && b instanceof PsiLiteralExpression)
					{
						...
						if(NullUtils.notNull(red, green, blue))
						{
							try
							{
								...
								{
									return manager.createProblemDescriptor(expression, "Convert to Gray._" + rr, new ConvertToGrayQuickFix(rr), ProblemHighlightType.GENERIC_ERROR_OR_WARNING,
											isOnTheFly);
								}
							}
					...
					if(e instanceof PsiLiteralExpression)
					{
						...
						if(literal != null)
						{
							try
							{
								...
								if(0 <= num && num < 256)
								{
									return manager.createProblemDescriptor(expression, "Convert to Gray_" + num, new ConvertToGrayQuickFix(num), ProblemHighlightType.GENERIC_ERROR_OR_WARNING,
											isOnTheFly);
								}
							}
							catch(Exception ignore)
