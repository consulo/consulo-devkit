...
public class PsiJavaHelper extends JavaHelper {
  ...
  private static boolean acceptsMethod(PsiMethod method, boolean staticMethods) {
    ...
    return staticMethods == modifierList.hasModifierProperty(PsiModifier.STATIC) && !modifierList.hasModifierProperty(PsiModifier.ABSTRACT) && (modifierList
      .hasModifierProperty(PsiModifier.PUBLIC) || !(modifierList.hasModifierProperty(PsiModifier.PROTECTED) || modifierList.hasModifierProperty(
      PsiModifier.PRIVATE)));
  }
