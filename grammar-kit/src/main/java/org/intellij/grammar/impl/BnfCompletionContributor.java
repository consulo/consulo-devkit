...
public class BnfCompletionContributor extends CompletionContributor {
    public BnfCompletionContributor() {
        PsiElementPattern.Capture<PsiElement> placePattern = psiElement()
        ...
        extend(
            ...
            placePattern.andNot(psiElement().inside(false, psiElement(BnfAttr.class))),
            (parameters, context, result) -> {
                ...
                for (BnfRule rule : file.getRules()) {
                    for (BnfReferenceOrToken element : SyntaxTraverser.psiTraverser(rule.getExpression())
                        .filter(BnfReferenceOrToken.class)) {
    ...
    private static boolean isLastInRuleOrFree(PsiElement element) {
        ...
        for (PsiElement cur = element, next =
             cur.getNextSibling(); next == null || next instanceof PsiComment || next instanceof PsiWhiteSpace;
    ...
    private static Collection<String> suggestKeywords(PsiElement position) {
        ...
        if (statement != null) {
            ...
            for (PsiElement cur = GrammarUtil.getDummyAwarePrevSibling(position); cur != null;
        ...
        if (completionOffset != 0) {
            TextRange altRange =
                TextRange.create(posRange.getEndOffset(), Math.min(posRange.getEndOffset() + 100, posFile.getTextLength()));
