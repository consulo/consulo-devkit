...
public class ExpressionGeneratorHelper {
    ...
    public static void generateExpressionRoot(ExpressionHelper.ExpressionInfo info, ParserGenerator g) {
        ...
        for (String opCall : sortedOpCalls) {
            OperatorInfo operator =
                ContainerUtil.getFirstItem(findOperators(
                    opCalls.get(opCall),
                    OperatorType.BINARY,
                    OperatorType.N_ARY,
            ...
            if (operator.type == OperatorType.BINARY) {
                String argCall =
                    format("%s(%s, %s, %d)", methodName, g.N.builder, g.N.level, rightAssociative ? argPriority - 1 : argPriority);
        ...
        for (String opCall : sortedOpCalls) {
            for (OperatorInfo operator : opCalls.get(opCall)) {
                if (operator.type == OperatorType.ATOM) {
                    if (Rule.isExternal(operator.rule)) {
                        continue;
                    }
                    g.newLine();
    ...
    @Nullable
    public static ConsumeType fixForcedConsumeType(
        @Nonnull ExpressionHelper expressionHelper,
        ...
        if (operatorInfo != null) {
            ...
            if (PsiTreeUtil.isAncestor(operatorInfo.operator, node, false)) {
