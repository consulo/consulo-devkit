...
public class NameShortener {
    ...
    public void addImports(Collection<String> initialImports, Collection<String> includedClasses) {
        ...
        for (String item : initialImports) {
            ...
            for (String s : StringUtil.tokenize(item.replaceAll("\\s+", " "), TYPE_TEXT_SEPARATORS)) {
                ...
                if (!s.contains(".") || !s.equals(shorten(s))) {
                    continue;
                }
                if (myPackage.equals(StringUtil.getPackageName(s))) {
                    continue;
                }
                if (includedClasses.contains(StringUtil.getShortName(s))) {
                    continue;
                }
                if (wasStatic) {
                    s = "static " + s;
    ...
    public String shorten(String s) {
        ...
        for (String part : StringUtil.tokenize(new StringTokenizer(StringUtil.trimEnd(s, "..."), TYPE_TEXT_SEPARATORS, true))) {
            ...
            if (TYPE_TEXT_SEPARATORS.contains(part) ||
                ...
                if ("\"".equals(part) && offset > 0 && s.indexOf(offset - 1) != '\\') {
                    quoted = !quoted;
                }
                sb.append(part);
    ...
    @Nullable
    private static String getAnnotatedFQNAt(@Nonnull String s, int offset) {
        Ref<String> result = Ref.create();
        addTypeToImports(s.substring(offset), result::set, 0);
        return result.get();
    }

    public static void addTypeToImports(
        @Nullable String type,
        @Nonnull List<String> typeAnnotations,
    ...
    private static void addTypeToImports(@Nullable String s, @Nonnull Consumer<String> result, int forcedOffset) {
        if (s == null) {
            return;
        }
        boolean quoted = false;
    ...
    @Nonnull
    public static String getRawClassName(@Nonnull String name) {
