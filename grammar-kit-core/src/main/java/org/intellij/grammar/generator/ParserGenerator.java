...
public class ParserGenerator {
  ...
  public ParserGenerator(@Nonnull BnfFile psiFile,
    ...
    for (BnfRule rule : myFile.getRules()) {
      ...
      String stubName =
        StringUtil.isNotEmpty(stubClass) ? stubClass :
          implSuper.indexOf("<") < implSuper.indexOf(">") && !myJavaHelper.findClassMethods(myVersion,
                                                                                            implSuperRaw,
    ...
    for (BnfRule rule : ordered) {
      ...
      String adjustedSuperRuleClass =
        StringUtil.isEmpty(stubName) ? superRuleClass :
          PlatformClass.AST_WRAPPER_PSI_ELEMENT.select(myVersion).equals(superRuleClass) ? PlatformClass.STUB_BASED_PSI_ELEMENT_BASE.select(
            myVersion) + "<" + stubName + ">" :
      ...
      info.mixedAST = topInfo != null ? topInfo.mixedAST : JBIterable.of(superRuleClass, info.realSuperClass)
                                                                     ...
                                                                     .flatMap(s -> JBTreeTraverser.<String>from(o -> JBIterable.of(
                                                                       myJavaHelper.getSuperClassName(o))).withRoot(s).unique())
                                                                     .find(s -> s.equals(PlatformClass.COMPOSITE_PSI_ELEMENT.select(
                                                                       myVersion))) != null;
    }
  }
  ...
  private void generateVisitor(String psiClass, Map<String, BnfRule> sortedRules) {
    String superIntf = ObjectUtil.notNull(ContainerUtil.getFirstItem(getRootAttribute(myVersion, myFile, KnownAttribute.IMPLEMENTS)),
                                          KnownAttribute.IMPLEMENTS.getDefaultValue(myVersion).get(0)).second.getStringValue(myVersion);
    Set<String> imports =
  ...
  @Nullable
  private NodeCall generateTokenChoiceCall(@Nonnull List<BnfExpression> children,
  ...
  @Nonnull
  NodeCall generateNodeCall(@Nonnull BnfRule rule,
  ...
  @Nonnull
  private ConsumeType getEffectiveConsumeType(@Nonnull BnfRule rule,
                                              @Nullable BnfExpression node,
                                              @Nullable ConsumeType forcedConsumeType) {
  ...
  private void generateElementTypesHolder(String className, Map<String, BnfRule> sortedCompositeTypes) {
    ...
    if (G.generatePsi && G.generatePsiFactory) {
      ...
      for (String elementType : sortedCompositeTypes.keySet()) {
        ...
        if (first1) {
          out("public static %s createElement(%s node) {",
  ...
  private void collectMethodTypesToImport(@Nonnull List<NavigatablePsiElement> methods, boolean isInPsiUtil, @Nonnull Set<String> result) {
    for (NavigatablePsiElement method : methods) {
