...
public class ParserGenerator {
    ...
    public ParserGenerator(
        ...
        myParserUtilClass = getRootAttribute(myVersion, myFile, KnownAttribute.PARSER_UTIL_CLASS);
        myPsiImplUtilClass = getRootAttribute(myVersion, myFile, KnownAttribute.PSI_IMPL_UTIL_CLASS);
        myPsiTreeUtilClass = getRootAttribute(myVersion, myFile, KnownAttribute.PSI_TREE_UTIL_CLASS);
        String tmpVisitorClass = getRootAttribute(myVersion, myFile, KnownAttribute.PSI_VISITOR_NAME);
        ...
        myVisitorClassName = tmpVisitorClass == null || !tmpVisitorClass.equals(StringUtil.getShortName(tmpVisitorClass))
            ? tmpVisitorClass
            : getRootAttribute(myVersion, myFile, KnownAttribute.PSI_PACKAGE) + "." + tmpVisitorClass;
        myTypeHolderClass = getRootAttribute(myVersion, myFile, KnownAttribute.ELEMENT_TYPE_HOLDER_CLASS);
        ...
        for (BnfRule r : rules) {
            ...
            myRuleInfos.put(
                ...
                new RuleInfo(
                    ...
                    Rule.isFake(r),
                    getElementType(r),
                    getAttribute(myVersion, r, KnownAttribute.PARSER_CLASS),
                    noPsi ? null : getAttribute(myVersion, r, KnownAttribute.PSI_PACKAGE),
                    noPsi ? null : getAttribute(myVersion, r, KnownAttribute.PSI_IMPL_PACKAGE),
                    noPsi ? null : getRulePsiClassName(r, myIntfClassFormat),
                    noPsi ? null : getRulePsiClassName(r, myImplClassFormat),
                    noPsi ? null : getAttribute(myVersion, r, KnownAttribute.MIXIN),
                    noPsi ? null : getAttribute(myVersion, r, KnownAttribute.STUB_CLASS)
                )
            );
        }
    ...
    private void calcAbstractRules() {
        ...
        for (BnfRule rule : myFile.getRules()) {
            String elementType = getAttribute(myVersion, rule, KnownAttribute.ELEMENT_TYPE);
        ...
        for (BnfRule rule : myFile.getRules()) {
            if (reusedRules.contains(rule.getName())) {
                continue;
            }
            if (myGrammarRoot.equals(rule.getName())) {
                continue;
            }
            if (!rule.getModifierList().isEmpty()) {
                continue;
            }
            if (getAttribute(myVersion, rule, KnownAttribute.RECOVER_WHILE) != null) {
                continue;
            }
            if (!getAttribute(myVersion, rule, KnownAttribute.HOOKS).isEmpty()) {
                continue;
            }
    ...
    private void calcFakeRulesWithType() {
        for (BnfRule rule : myFile.getRules()) {
            BnfRule r = myFile.getRule(getAttribute(myVersion, rule, KnownAttribute.ELEMENT_TYPE));
            if (r == null) {
                continue;
            }
    ...
    private void calcRulesStubNames() {
        for (BnfRule rule : myFile.getRules()) {
            ...
            BnfRule topSuper = getEffectiveSuperRule(myFile, rule);
            String superRuleClass = topSuper == null ? getRootAttribute(myVersion, myFile, KnownAttribute.EXTENDS) :
                topSuper == rule ? getAttribute(myVersion, rule, KnownAttribute.EXTENDS) :
            ...
            String stubName =
                StringUtil.isNotEmpty(stubClass) ? stubClass :
                    implSuper.indexOf("<") < implSuper.indexOf(">") && !myJavaHelper.findClassMethods(
                        myVersion,
                        implSuperRaw,
                        ...
                        ? implSuper.substring(implSuper.indexOf("<") + 1, implSuper.indexOf(">"))
                        : null;
    ...
    private void calcRealSuperClasses(Map<String, BnfRule> sortedPsiRules) {
        ...
        for (BnfRule rule : ordered) {
            ...
            String superRuleClass = topSuper == null
                ? getRootAttribute(myVersion, myFile, KnownAttribute.EXTENDS)
                : topSuper == rule
                ? getAttribute(myVersion, rule, KnownAttribute.EXTENDS)
                : topInfo.implClass;
            String stubName = info.realStubClass;
            String adjustedSuperRuleClass = StringUtil.isEmpty(stubName)
                ? superRuleClass
                : PlatformClass.AST_WRAPPER_PSI_ELEMENT.select(myVersion).equals(superRuleClass)
                ? PlatformClass.STUB_BASED_PSI_ELEMENT_BASE.select(myVersion) + "<" + stubName + ">"
    ...
    @Nonnull
    public String shorten(@Nonnull String s) {
    ...
    public void generate(Set<GenerateTarget> targets) throws IOException {
        ...
        for (BnfRule rule : myFile.getRules()) {
            ...
            if (sortedCompositeTypes.containsKey(elementType)) {
                continue;
            }
    ...
    private void generateVisitor(String psiClass, Map<String, BnfRule> sortedRules) {
        String superIntf = ObjectUtil.notNull(
            ContainerUtil.getFirstItem(getRootAttribute(myVersion, myFile, KnownAttribute.IMPLEMENTS)),
    ...
    public void generateParser(String parserClass, Collection<String> ownRuleNames) {
        List<String> parserImports = getRootAttribute(myVersion, myFile, KnownAttribute.PARSER_IMPORTS).asStrings(myVersion);
        boolean rootParser = parserClass.equals(myGrammarRootParser);
        Set<String> imports = new LinkedHashSet<>();
        if (!G.generateFQN) {
            imports.add(PlatformClass.PSI_BUILDER.select(myVersion));
            imports.add(PlatformClass.LANGUAGE_VERSION.select(myVersion));
        ...
        if (!rootParser) {
            ...
            imports.addAll(Arrays.asList(
                PlatformClass.IELEMENT_TYPE.select(myVersion),
        ...
        for (String ruleName : ownRuleNames) {
            ...
            if (Rule.isExternal(rule) || Rule.isFake(rule)) {
                continue;
            }
            if (myExpressionHelper.getExpressionInfo(rule) != null) {
                continue;
            }
            out("/* ********************************************************** */");
    ...
    private void generateParserLambdas(@Nonnull String parserClass) {
    ...
    @Nonnull
    private String generateParserInstance(@Nonnull String body) {
    ...
    private void generateRootParserContent() {
        ...
        for (String ruleName : myRuleInfos.keySet()) {
            BnfRule rule = Objects.requireNonNull(myFile.getRule(ruleName));
            if (getAttribute(myVersion, rule, KnownAttribute.ELEMENT_TYPE) != null) {
                continue;
            }
            if (!RuleGraphHelper.hasElementType(rule)) {
                continue;
            }
            if (Rule.isFake(rule) || Rule.isMeta(rule)) {
                continue;
            }
            ExpressionHelper.ExpressionInfo info = myExpressionHelper.getExpressionInfo(rule);
            if (info != null && info.rootRule != rule) {
            ...
            if (!Boolean.TRUE.equals(getAttribute(myVersion, rule, KnownAttribute.EXTRA_ROOT))) {
                continue;
            }
            extraRoots.add(rule);
        }
        ...
        String shortLV = shorten(PlatformClass.LANGUAGE_VERSION);
        ...
        if (extraRoots.isEmpty()) {
            ...
            for (BnfRule rule : extraRoots) {
                ...
                String nodeCall = generateNodeCall(ObjectUtil.notNull(rootRule, rule), null, rule.getName()).render(N);
        ...
        for (Map.Entry<BnfRule, Collection<BnfRule>> entry : map.entrySet()) {
            ...
            for (BnfRule rule : entry.getValue()) {
                ...
                String elementType = info.isFake && !info.isInElementType ||
                    getSynonymTargetOrSelf(myVersion, rule) != rule ? null : info.elementType;
                if (StringUtil.isEmpty(elementType)) {
                    continue;
                }
                if (set == null) {
    ...
    private void generateClassHeader(
        ...
        for (RuleInfo info : myRuleInfos.values()) {
            if (includedPackages.contains(info.intfPackage)) {
                includedClasses.add(StringUtil.getShortName(info.intfClass));
            }
            if (includedPackages.contains(info.implPackage)) {
                includedClasses.add(StringUtil.getShortName(info.implClass));
            }
        }
    ...
    private void generateFileHeader(String className) {
        String header = getRootAttribute(myVersion, myFile, KnownAttribute.CLASS_HEADER, className);
     ...
     */
    private void generateMetaMethod(@Nonnull String methodName, @Nonnull List<String> parameterNames, boolean isRule) {
    ...
    void generateNode(BnfRule rule, BnfExpression initialNode, String funcName, Set<BnfExpression> visited) {
        ...
        String recoverWhile = !firstNonTrivial ? null : getAttribute(myVersion, rule, KnownAttribute.RECOVER_WHILE);
        ...
        String frameName =
            !children.isEmpty() && firstNonTrivial && !Rule.isMeta(rule) ? quote(getRuleDisplayName(myVersion, rule, !isPrivate)) : null;

        String extraParameters = metaParameters.stream().map(it -> ", Parser " + it).collect(Collectors.joining());
        out(
            "%sstatic boolean %s(%s %s, int %s%s) {",
            !isRule ? "private " : isPrivate ? "" : "public ",
            funcName,
            shorten(PlatformClass.PSI_BUILDER),
            N.builder,
            N.level,
            extraParameters
        );
        ...
        if (recoverWhile == null && (isRule || firstNonTrivial)) {
            frameName = generateFirstCheck(rule, frameName, getAttribute(myVersion, rule, KnownAttribute.NAME) == null);
        }
    ...
    public String generateFirstCheck(BnfRule rule, String frameName, boolean skipIfOne) {
        ...
        for (BnfExpression expression : firstSet) {
            if (expression == BNF_MATCHES_EOF || expression == BNF_MATCHES_ANY) {
                return frameName;
            }

            String expressionString = BnfFirstNextAnalyzer.asString(expression);
            if (myFile.getRule(expressionString) != null) {
                continue;
            }
        ...
        if (allTokensCount <= G.generateFirstCheck) {
            ...
            String condition = grouped.entrySet().stream().map(entry -> {
                ...
                if (!dropFrameName && consumeType == ConsumeType.DEFAULT) {
                    sb.append(StringUtil.notNullize(frameName, "\"\"")).append(", ");
                }
                appendTokenTypes(sb, tokenTypes);
                sb.append(")");
                return sb;
        ...
        return dropFrameName && StringUtil.isEmpty(getAttribute(myVersion, rule, KnownAttribute.NAME)) ? null : frameName;
    }

    @Nonnull
    private ConsumeType getRuleConsumeType(@Nonnull BnfRule rule, @Nullable BnfRule contextRule) {
        ...
        return ObjectUtil.chooseNotNull(forcedConsumeType, ConsumeType.forRule(myVersion, rule));
    }
    ...
    void generateNodeChild(BnfRule rule, BnfExpression child, String funcName, int index, Set<BnfExpression> visited) {
        ...
        else if (GrammarUtil.isAtomicExpression(child) || isTokenSequence(myVersion, rule, child)) {
            // do not generate
        }
        else {
    ...
    @Nonnull
    private List<String> collectMetaParametersFormatted(@Nonnull BnfRule rule, @Nullable BnfExpression expression) {
        if (expression == null) {
            return Collections.emptyList();
        }
    ...
    @Nonnull
    private String formatMetaParamName(@Nonnull String s) {
        String argName = s.trim();
        return N.metaParamPrefix + (N.metaParamPrefix.isEmpty() || "_".equals(N.metaParamPrefix)
            ? argName
            : StringUtil.capitalize(argName));
    }
    ...
    @Nonnull
    @RequiredReadAction
    private NodeCall generateTokenSequenceCall(
        List<BnfExpression> children,
        int startIndex,
        PinMatcher pinMatcher,
        boolean pinApplied,
        int[] skip,
        @Nonnull NodeCall nodeCall,
        boolean rollbackOnFail,
        ConsumeType consumeType
    ) {
        if (startIndex == children.size() - 1 || !(nodeCall instanceof ConsumeTokenCall)) {
            return nodeCall;
        }
    ...
    @Nullable
    @RequiredReadAction
    private NodeCall generateTokenChoiceCall(
        @Nonnull List<BnfExpression> children,
        @Nonnull ConsumeType consumeType,
    ...
    @Nonnull
    @RequiredReadAction
    NodeCall generateNodeCall(@Nonnull BnfRule rule, @Nullable BnfExpression node, @Nonnull String nextName) {
        return generateNodeCall(rule, node, nextName, null);
    }

    @Nonnull
    @RequiredReadAction
    NodeCall generateNodeCall(
        @Nonnull BnfRule rule,
        @Nullable BnfExpression node,
        @Nonnull String nextName,
        ...
        if (type == BNF_STRING) {
            ...
            return generateConsumeTextToken(
                consumeType,
                StringEscapeUtil.isQuoted(text, '"') ? value : StringEscapeUtil.escape(value, '"')
            );
        }
        ...
        else if (isTokenSequence(myVersion, rule, node)) {
            ConsumeType consumeType = getEffectiveConsumeType(rule, node, forcedConsumeType);
            PinMatcher pinMatcher = new PinMatcher(myVersion, rule, type, nextName);
    ...
    @Nonnull
    private ConsumeType getEffectiveConsumeType(
        @Nonnull BnfRule rule,
        @Nullable BnfExpression node,
        @Nullable ConsumeType forcedConsumeType
    ) {
        if (forcedConsumeType == ConsumeType.DEFAULT) {
            return ConsumeType.DEFAULT;
        }
        PsiElement parent = node == null ? null : node.getParent();

        if (forcedConsumeType == null && parent instanceof BnfSequence bnfSequence
            && ContainerUtil.getFirstItem(bnfSequence.getExpressionList()) != node) {
            Set<BnfExpression> expressions = new BnfFirstNextAnalyzer()
                .setParentFilter(o -> o != parent)
                .calcFirst(bnfSequence);
        ...
        return fixed != null ? fixed : ConsumeType.forRule(myVersion, rule);
    }

    @Nonnull
    @RequiredReadAction
    private NodeCall generateExternalCall(@Nonnull BnfRule rule, @Nonnull List<BnfExpression> expressions, @Nonnull String nextName) {
        ...
        if (targetRule != null) {
            if (Rule.isExternal(targetRule)) {
                ...
                if (metaParameterNames.size() < expressions.size() - 1) {
                    callParameters =
                        ContainerUtil.concat(callParameters, expressions.subList(metaParameterNames.size() + 1, expressions.size()));
                }
            }
            else {
    ...
    private String getMetaMethodFieldRef(@Nonnull String call, @Nonnull String nextName) {
    ...
    @Nonnull
    private String getParserLambdaRef(@Nonnull NodeCall nodeCall, @Nonnull String nextName) {
    ...
    @Nonnull
    private NodeCall generateConsumeToken(@Nonnull ConsumeType consumeType, @Nonnull String tokenName) {
        myTokensUsedInGrammar.add(tokenName);
        return new ConsumeTokenCall(consumeType, getElementType(tokenName));
    }

    @Nonnull
    public static NodeCall generateConsumeTextToken(@Nonnull ConsumeType consumeType, @Nonnull String tokenText) {
    ...
    String getElementType(BnfRule r) {
        return ParserGeneratorUtil.getElementType(myVersion, r, G.generateElementCase);
    }

    /*ElementTypes******************************************************************/

    private void generateElementTypesHolder(String className, Map<String, BnfRule> sortedCompositeTypes) {
        String tokenTypeClass = getRootAttribute(myVersion, myFile, KnownAttribute.TOKEN_TYPE_CLASS);
        String tokenTypeFactory = getRootAttribute(myVersion, myFile, KnownAttribute.TOKEN_TYPE_FACTORY);
        Set<String> imports = new LinkedHashSet<>();
        ...
        if (G.generateTokenTypes) {
            ...
            String fieldType = ObjectUtil.notNull(useExactTokens ? exactType : null, PlatformClass.IELEMENT_TYPE.select(myVersion));
            for (String tokenText : mySimpleTokens.keySet()) {
                String tokenName = ObjectUtil.chooseNotNull(mySimpleTokens.get(tokenText), tokenText);
                if (isIgnoredWhitespaceToken(tokenName, tokenText)) {
                    continue;
                }
            ...
            for (String tokenType : sortedTokens.keySet()) {
                ...
                out(
                    ...
                    tokenType,
                    tokenCreateCall,
                    StringEscapeUtil.escape(tokenString, '"'),
                    callFix
                );
            }
            generateTokenSets();
    ...
    private void generateElementTypesHolderFactory(
        String className,
        ...
        if (asMapFactory) {
            ...
            out("}");
            newLine();
            out("public static %s<%s> elementTypes() {", shorten(Set.class.getName()), shortET);
            ...
            for (String elementType : sortedCompositeTypes.keySet()) {
                ...
                if (first1) {
                    out(
                        "public static %s createElement(%s node) {",
                        shorten(PlatformClass.PSI_ELEMENT.select(myVersion)),
                        shorten(PlatformClass.AST_NODE)
                    );
                    out("%s type = node.getElementType();", shorten(PlatformClass.IELEMENT_TYPE));
                }
                String psiClass =
                    getAttribute(myVersion, rule, KnownAttribute.PSI_IMPL_PACKAGE) + "." + getRulePsiClassName(rule, myImplClassFormat);
    ...
    private void generateTokenSets() {
        ...
        myTokenSets.forEach((name, tokens) -> {
            ...
            out("TokenSet %s = %s;", name, ObjectUtil.chooseNotNull(alreadyRendered, value));
        });
        out("}");
    }
    ...
    private void generatePsiImpl(BnfRule rule, RuleInfo info) {
        ...
        Function<String, String> substitutor = stubName == null ? genericUnwrapper : o -> {
            String oo = genericUnwrapper.apply(o);
        ...
        for (BnfRule next = rule; next != null && next != topSuperRule; ) {
            ...
            if (next != null && next != topSuperRule && getAttribute(myVersion, topSuperRule, KnownAttribute.MIXIN) == null) {
                continue;
            }
        ...
        if (!G.generateTokenTypes) {
            ...
            for (RuleMethodsHelper.MethodInfo methodInfo : myRulesMethodsHelper.getFor(rule)) {
                if (methodInfo.rule == null && !StringUtil.isEmpty(methodInfo.name)) {
                    for (String s : getRootAttribute(myVersion, myFile, KnownAttribute.PARSER_IMPORTS).asStrings(myVersion)) {
                ...
                case MIXIN:
                    ...
                    if (intf) {
                        ...
                        List<NavigatablePsiElement> methods =
                            myJavaHelper.findClassMethods(myVersion, mixinClass, JavaHelper.MethodType.INSTANCE, methodInfo.name, -1);
                    ...
                    List<NavigatablePsiElement> methods =
                        findRuleImplMethods(myVersion, myJavaHelper, myPsiImplUtilClass, methodInfo.name, rule);
                    for (NavigatablePsiElement method : methods) {
    ...
    public Collection<String> getRuleMethodTypesToImport(BnfRule rule) {
        ...
        for (RuleMethodsHelper.MethodInfo methodInfo : methods) {
            if (methodInfo.type != RuleMethodsHelper.MethodType.MIXIN) {
                continue;
            }
    ...
    private void collectMethodTypesToImport(
        @Nonnull List<NavigatablePsiElement> methods,
        boolean isInPsiUtil,
        ...
        for (NavigatablePsiElement method : methods) {
            ...
            for (JavaHelper.TypeParameterInfo generic : myJavaHelper.getGenericParameters(method)) {
                for (String superType : generic.getExtendsList()) {
                    addTypeToImports(superType, List.of(), result);
                }
            }
            ...
            for (String exception : myJavaHelper.getExceptionList(method)) {
                addTypeToImports(exception, List.of(), result);
            }
        }
    }
    ...
    @RequiredReadAction
    private String generatePsiAccessorImplCall(@Nonnull BnfRule rule, @Nonnull RuleMethodsHelper.MethodInfo methodInfo, boolean mixedAST) {
        boolean isToken = methodInfo.rule == null;
        ...
        if (!mixedAST && myNoStubs) {
            if (isToken) {
                ...
                return many
                    ? String.format("%s.getChildrenOfTypeAsList(this, %s.class)", shorten(myPsiTreeUtilClass), className)
        ...
        if (isToken) {
            ...
            String getterName = stubbed && many ? "getStubChildrenOfTypeAsList"
                ...
                : many ? "getChildrenOfTypeAsList"
                : "getChildOfType";
            result = String.format("%s.%s(this, %s.class)", shorten(myPsiTreeUtilClass), getterName, className);
        }
    ...
    private String getAccessorType(@Nonnull BnfRule rule) {
        if (Rule.isExternal(rule)) {
            Pair<KnownAttribute.ListValueObject, KnownAttribute.ListValueObject> first =
                ContainerUtil.getFirstItem(getAttribute(myVersion, rule, KnownAttribute.IMPLEMENTS));
            return Objects.requireNonNull(first).second.getStringValue(myVersion);
        }
        else {
    ...
    private JBIterable<?> resolveUserPsiPathMethods(
        BnfRule startRule,
        String[] splitPath
    ) {
    ...
    private void generateUserPsiAccessors(BnfRule startRule, RuleMethodsHelper.MethodInfo methodInfo, boolean intf, boolean mixedAST) {
        ...
        for (Object m : resolveUserPsiPathMethods(startRule, splitPath)) {
            ...
            String className =
                shorten(targetInfo.rule == null ? PlatformClass.PSI_ELEMENT.select(myVersion) : getAccessorType(targetInfo.rule));
    ...
    private void generateUtilMethod(
        String methodName,
        ...
        int offset = methodTypes.isEmpty() || isInPsiUtil && methodTypes.size() < 3 ? 0 : isInPsiUtil ? 3 : 1;
        ...
        if (!intf /*|| hasMethodInInfos*/) {
            out(shorten(OVERRIDE_ANNO));
        }
        for (String s : myJavaHelper.getAnnotations(method)) {
            if ("java.lang.Override".equals(s)) {
                continue;
            }
