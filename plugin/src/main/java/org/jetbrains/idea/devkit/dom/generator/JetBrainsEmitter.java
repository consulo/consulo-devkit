        ...
        try {
            ...
            if (td.type != TypeDesc.TypeEnum.ENUM) {
                ...
                if (!model.getNSDPrefix("", nsd.superClass, false).equals(model.getNSDPrefix(td))) {
                    externalClasses.add(
                        model.getNSDPrefix("", nsd.superClass, false) +
            ...
            for (FieldDesc field : fields) {
                ...
                if (field.clType == FieldDesc.SIMPLE) {
                    ...
                    if (field.duplicateIndex >= 0) {
                        ...
                        sbAnnotations.append((JB_OFF ? "//" : "") + "\t@SubTag (value = \"")
                            .append(tagName)
                            .append("\", indicator = true)");
                    }
        ...
        try {
            ...
            for (TypeDesc td : jtList) {
                ...
                for (FieldDesc fd : fields) {
                    ...
                    else if ((fd.clType == -FieldDesc.OBJ || fd.clType == -FieldDesc.STR)
                        && fd.name.endsWith("displayNames") && guessPriority < 5) {
                        guessPriority = 5;
                        guessedField = fd;
                    }
                }
